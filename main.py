...
for i in range(len(Pset)):
    for j in range(len(Pset_bar)):
        P = Pset[i]
        P_bar = Pset_bar[j]  
        chi[i,j] = chi2measure(corpusa = P[0], corpusb = P_bar[0], N=1000, remove_punctuation = True)
        print(j)
...
plt.bar(hist[1][:-1] + delta/2, hist[0]*delta ,delta*spacing ,align='edge', color=(.9,.9,.9), edgecolor='black')
...
plt.bar(hist[1][:-1]+offset , hist[0]*delta, delta*spacing, align='edge', color=(.6,.6,.6), edgecolor='black')
...
arr = {'Selection method': ('Optimal','EMD','KLD','CMEK','MMD','Chi2','Random'),
...
np.savetxt('table1.txt',table1,fmt='%.3f')
...
from classify import cross_error
from astropy.table import Table
from significance_ttset_all import avg_Nset_error
...
X1 = normalize(np.stack((s0,s1,s2,s3,s4), axis=1),axis=0)                 
X2 = normalize(s0,axis=1).transpose() 
X3 = normalize(s2,axis=1).transpose() 
Xset = [X1,X2,X3] 
...
plt.plot(range(1,m),Q[:,0],linestyle="--",color=(0,0,0))
...
plt.plot(range(1,m),(m-1)*[np.mean(xi_cross_all)],linestyle=":",color=(.5,.5,.5),zorder=1)
...
plt.legend(('CMEK selection','Chi2 selection','EMD selection','random selection','training on all domains'))
