...
#%% Chi2
def chi2measure(corpusa, corpusb, N, remove_punctuation):
    from measures import remove_punct
    from measures import count_freq
    from measures import get_eval_voc
    
    if remove_punctuation:
        corpusa = remove_punct(corpusa)
        corpusb = remove_punct(corpusb)
    
    [wordsa,freqa] = count_freq(corpusa)
    [wordsb,freqb] = count_freq(corpusb)
    
    (eval_voc,n) = get_eval_voc(wordsa,wordsb,N)
     
    lista = []
    listb = []
    
    for word in eval_voc:
        if word in wordsa:
            lista.append(freqa[wordsa.index(word)])
        else:
            lista.append(0)
        if word in wordsb:
            listb.append(freqb[wordsb.index(word)])
        else:
            listb.append(0)
    ...
    lista = [i*fa for i in lista]
    listb = [i*fb for i in listb]   
    chi = 0  
    
    for i in range(0,len(lista)):
        chi = chi + (lista[i]-listb[i])**2 / (lista[i]+listb[i])
