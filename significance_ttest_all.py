...
p = 0.385  #probability of Chi2 selection selecting the best domain
cdf = 0
for i in np.arange(1,n+1,1)[::-1]:
    pi = scipy.stats.binom.pmf(i,n,p)
    cdf = cdf + pi
...
p = 12./13  #probability of Chi2 selection not selecting one of 5 worst domains
cdf = 0
for i in np.arange(0,n+1,1)[::-1]:
    pi = scipy.stats.binom.pmf(i,n,p)
    cdf = cdf + pi
...
nd = 10   #number of domains evaluated -1
m = 13
